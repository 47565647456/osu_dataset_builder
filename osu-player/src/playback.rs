...
impl Plugin for PlaybackPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<PlaybackStateRes>()
...
impl Default for PlaybackStateRes {
    fn default() -> Self {
        Self {
...
impl PlaybackStateRes {
    ...
    /// Update time manually (when no audio)
...
/// System to update playback time
fn update_playback_time(
    mut playback: ResMut<PlaybackStateRes>,
    beatmap: Res<crate::beatmap::BeatmapView>,
) {
    // Initialize total duration if not set
    if playback.total_duration == 0.0 {
        playback.total_duration = beatmap.total_duration;
    }
    
    // For now, use manual time tracking
    // Audio sync can be added when audio is properly loaded
    playback.update_manual();
}
